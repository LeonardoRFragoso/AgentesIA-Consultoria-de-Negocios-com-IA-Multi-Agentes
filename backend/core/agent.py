...
class BaseAgent(ABC):
    ...
    def _load_prompt(self, context: Optional[ExecutionContext] = None) -> str:
        """
        Carrega prompt do arquivo e renderiza template Jinja2.
        
        Args:
            context: Contexto de execução com business_type e analysis_depth
        """
        try:
            with open(self.prompt_path, "r", encoding="utf-8") as f:
                template_str = f.read()
            
            # Se tiver contexto, renderiza template com variáveis
            if context:
                from jinja2 import Template
                
                # Descrição da profundidade
                depth_descriptions = {
                ...
                return template.render(
                    ...
                    depth_description=depth_descriptions.get(context.analysis_depth, "análise padrão"),
                )
    ...
    async def _execute_internal(self, context: ExecutionContext) -> str:
        ...
        """
        # Carrega prompt com variáveis renderizadas
        system_prompt = self._load_prompt(context)
        ...
        if not api_key:
            raise AgentExecutionError(
                self.name, 
                "ANTHROPIC_API_KEY não configurada. Adicione no arquivo .env",
                None
            )
        
        try:
            # Cria cliente sem argumentos extras para evitar conflitos de versão
            ...
            raise AgentExecutionError(self.name, f"Erro na API Anthropic: {str(e)}", e)
        except Exception as e:
            if "proxies" in str(e):
                # Problema de versão do httpx/anthropic
                raise AgentExecutionError(
                    self.name,
                    "Erro de versão: execute 'pip install --upgrade anthropic httpx' para corrigir",
                    e
                )
            raise
